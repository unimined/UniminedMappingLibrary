...
object LegacyATReader : FormatReader {
    ...
    fun readData(input: CharReader<*>, leinient: Boolean = LegacyATReader.leinient): List<ATReader.ATItem> {
        ...
        while (!input.exhausted()) {
            ...
            val memberName = if (input.peek() != '.') null else input.takeUntil { it.isWhitespace() || it == '(' }.ifBlank {
                throw IllegalArgumentException("Expected member name")
            }.substring(1)
            val memberDesc = if (memberName == null) null else input.takeUntil { it.isWhitespace() }.ifBlank { null }
